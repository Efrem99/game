...
void PhysicsEngine::applyJump(CharacterState& cs, float force) {
    if (!cs.grounded && !cs.inWater) return;
    cs.velocity.z = force;
    cs.grounded   = false;
}
...
void PhysicsEngine::step(CharacterState& cs, float dt) {
    ...
    if (!cs.grounded && !cs.inWater) {
        ...
        // Terminal velocity
        if (cs.velocity.z < -TERM_VEL) cs.velocity.z = -TERM_VEL;
    }
    ...
    if (cs.comboTimer > 0.f) {
        cs.comboTimer -= dt;
        if (cs.comboTimer <= 0.f) cs.comboCount = 0;
    }
}
...
bool PhysicsEngine::resolveGround(CharacterState& cs) {
    ...
    if (cs.position.z < 0.f && !cs.inWater) {
        cs.position.z = 0.f;
        if (cs.velocity.z < 0) cs.velocity.z = 0.f;
        cs.grounded = true;
        return true;
    }
    ...
    if (cs.position.z > 0.05f) cs.grounded = false;
    return false;
}
...
bool PhysicsEngine::resolveWater(CharacterState& cs) {
    ...
    for (auto& p : _platforms) {
        ...
        if (cs.position.x >= p.aabb.min.x && cs.position.x <= p.aabb.max.x &&
            ...
            if (cs.inWater) cs.grounded = false;
            return cs.inWater;
        }
    }
    cs.inWater    = false;
...
bool PhysicsEngine::isInWater(const Vec3& pos) const {
    for (auto& p : _platforms) {
        ...
        if (pos.x >= p.aabb.min.x && pos.x <= p.aabb.max.x &&
            ...
            pos.z <= p.aabb.max.z) return true;
    }
    return false;
}
