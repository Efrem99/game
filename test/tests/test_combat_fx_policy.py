...
class CombatFxPolicyTests(unittest.TestCase):
    ...
    def test_pick_first_existing_texture_path_skips_opaque_candidates_when_alpha_required(self):
        chosen = pick_first_existing_texture_path(
            ["opaque.png", "alpha.png"],
            exists_fn=lambda p: p in {"opaque.png", "alpha.png"},
            require_alpha=True,
            alpha_ok_fn=lambda p: p == "alpha.png",
        )
        self.assertEqual("alpha.png", chosen)

    def test_load_optional_texture_uses_fallback_texture_when_alpha_candidates_are_invalid(self):
        class _Loader:
            def loadTexture(self, path):
                return f"loaded:{path}"

        chosen = load_optional_texture(
            _Loader(),
            candidates=["opaque.png"],
            exists_fn=lambda p: p == "opaque.png",
            require_alpha=True,
            alpha_ok_fn=lambda _p: False,
            fallback_texture=lambda: "generated-alpha",
        )
        self.assertEqual("generated-alpha", chosen)
