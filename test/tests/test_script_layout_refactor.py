...
class ScriptLayoutRefactorTests(unittest.TestCase):
    ...
    def test_themed_animation_report_resolves_repo_root_from_nested_script_home(self):
        themed_report = ROOT / "scripts" / "animation" / "player_anim_runtime_report.py"
        source = themed_report.read_text(encoding="utf-8")

        self.assertIn('ROOT = Path(__file__).resolve().parents[2]', source)
        self.assertIn('SRC = ROOT / "src"', source)
